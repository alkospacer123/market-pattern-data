...
DEFAULT_OUTPUT = ROOT / "results" / "cycle36_roll30_rejection_generalization"
ENGINE = ROOT / "research" / "cycle36_roll30_rejection_generalization.py"
...
def run_one(data_root: Path, output_root: Path, instrument: str) -> dict:
    ...
    print("\n" + "=" * 78, flush=True)
    print(f"CYCLE36 {instrument}", flush=True)
    print("=" * 78, flush=True)
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"Cycle36 {instrument} failed rc={proc.returncode}; see {log_path}")
...
def main() -> None:
    ...
    summary = {
        ...
        "protocol": "research/CYCLE36_PROTOCOL.md",
        "engine": "research/cycle36_roll30_rejection_generalization.py",
        "results": results,
    }
    summary_path = a.output / "cycle36_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")

    print("\n" + "=" * 78, flush=True)
    print("CYCLE36 FINISHED", flush=True)
