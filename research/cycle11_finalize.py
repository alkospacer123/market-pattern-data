...
def run(block_root:Path,out:Path):
    ...
    all_g=[];all_b=[];all_s=[];by_inst={"CNYRUBF":{"g":[],"b":[],"s":[]},"USDRUBF":{"g":[],"b":[],"s":[]}};profile_counts={};model_tail=[]
    for block in blocks:
        if block.get("retired_internal_confirmation_accessed") or block.get("true_oos_2025_accessed"):raise PermissionError("data fence violation in block")
        for d in block.get("model_diagnostics",[]):
            model_tail.append({"block_index":block["block_index"],**d})
        for ir in block["instruments"]:
            inst=ir["instrument"];g=ir["gross_trades"];b=ir["base_trades"];s=ir["stress_trades"]
            all_g+=g;all_b+=b;all_s+=s;by_inst[inst]["g"]+=g;by_inst[inst]["b"]+=b;by_inst[inst]["s"]+=s
            for t in b:profile_counts[t["profile"]]=profile_counts.get(t["profile"],0)+1
    gm,bm,sm=v3.metrics(all_g),v3.metrics(all_b),v3.metrics(all_s);passed,fail=gate(gm,bm,sm)
    inst_rows=[]
    for inst,x in by_inst.items():
        inst_rows.append({"instrument":inst,"gross":v3.metrics(x["g"]),"base":v3.metrics(x["b"]),"stress":v3.metrics(x["s"]),"base_months":month_metrics(x["b"]),"stress_months":month_metrics(x["s"])})
    result={"engine":"cycle11-nonlinear-pnl-v1","status":"RESEARCH_SURVIVOR_AWAITING_NEW_CONFIRMATION" if passed else "NO_RESEARCH_SURVIVOR","block_count":len(blocks),"data_start":"2026-03-02","data_end_exclusive":"2026-05-16","retired_internal_confirmation_accessed":False,"true_oos_2025_accessed":False,"commission_excluded":True,"gates":GATES,"gate_pass":passed,"gate_failures":fail,"combined":{"gross":gm,"base":bm,"stress":sm,"base_months":month_metrics(all_b),"stress_months":month_metrics(all_s)},"instruments":inst_rows,"profile_counts":profile_counts,"blocks":[{"block_index":b["block_index"],"start":b["block_start"],"end":b["block_end_exclusive"],"prediction_summary":b["prediction_summary"]} for b in blocks],"model_diagnostics":model_tail}
    ...
    lines=["# Cycle 11 — Nonlinear Causal Executable-PnL Discovery","",f"Status: **{result['status']}**",f"Combined GROSS PF={gm['pf']:.3f} exp={gm['expectancy_bps']} N={gm['trades']} total={gm['total_bps']:.3f} bps",f"Combined BASE PF={bm['pf']:.3f} exp={bm['expectancy_bps']} N={bm['trades']} days={bm['unique_days']} months+={bm['positive_months']} total={bm['total_bps']:.3f} bps",f"Combined STRESS PF={sm['pf']:.3f} exp={sm['expectancy_bps']} months+={sm['positive_months']} total={sm['total_bps']:.3f} bps",f"Gate={passed}; failures={fail}",f"Profile counts={profile_counts}",""]
    ...
    lines += ["","All weekly predictions are causal relative to their own block. This entire March-May period is nevertheless research data, not fresh OOS. Retired May16-Jul1 and 2025 were not read."]
