...
def run(root:Path,out:Path):
 ...
 agg={k:{'g':[],'b':[],'s':[]} for k in ('CNYRUBF','USDRUBF')};g=[];b=[];s=[];pc={}
 for bl in blocks:
 ...
 res={'engine':'cycle14-m1-micro-v1','status':'RESEARCH_SURVIVOR_AWAITING_NEW_CONFIRMATION' if passed else 'NO_RESEARCH_SURVIVOR','block_count':11,'data_start':'2026-03-02','data_end_exclusive':'2026-05-16','retired_internal_confirmation_accessed':False,'true_oos_2025_accessed':False,'commission_excluded':True,'gates':GATES,'gate_pass':passed,'gate_failures':fail,'combined':{'gross':gm,'base':bm,'stress':sm,'base_months':mm(b),'stress_months':mm(s)},'instruments':inst,'profile_counts':pc,'blocks':[{'block_index':z['block_index'],'start':z['block_start'],'end':z['block_end_exclusive'],'prediction_summary':z['prediction_summary']} for z in blocks]}
 ...
 lines=['# Cycle 14 — M1 Microstructure Executable-PnL Discovery','',f"Status: **{res['status']}**",f"GROSS PF={gm['pf']:.3f} exp={gm['expectancy_bps']} N={gm['trades']}",f"BASE PF={bm['pf']:.3f} exp={bm['expectancy_bps']} N={bm['trades']} months+={bm['positive_months']}",f"STRESS PF={sm['pf']:.3f} exp={sm['expectancy_bps']} months+={sm['positive_months']}",f"Gate={passed}; failures={fail}",f"Profiles={pc}",'']
