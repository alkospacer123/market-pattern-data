...
DEFAULT_OUTPUT = ROOT / "results" / "cycle34_exit_occupancy_compression"
ENGINE = ROOT / "research" / "cycle34_exit_occupancy_compression.py"
...
def run_one(data_root: Path, output_root: Path, instrument: str) -> dict:
    ...
    print("\n" + "=" * 78, flush=True)
    print(f"CYCLE34 {instrument}", flush=True)
    print("=" * 78, flush=True)
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"Cycle34 {instrument} failed rc={proc.returncode}; see {log_path}")
...
def main() -> None:
    ...
    summary = {
        ...
        "protocol": "research/CYCLE34_PROTOCOL.md",
        "engine": "research/cycle34_exit_occupancy_compression.py",
        "results": results,
    }
    summary_path = a.output / "cycle34_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")

    print("\n" + "=" * 78, flush=True)
    print("CYCLE34 FINISHED", flush=True)
